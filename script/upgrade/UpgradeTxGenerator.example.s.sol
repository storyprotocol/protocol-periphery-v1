 ...
 *      forge script script/upgrade/UpgradeTxGenerator.example.s.sol:UpgradeTxGeneratorExample --rpc-url=$STORY_RPC --private-key=$STORY_PRIVATEKEY
 */
contract UpgradeTxGeneratorExample is TxGenerator, StoryProtocolPeripheryAddressManager, StoryProtocolCoreAddressManager {
    constructor() TxGenerator(
        "vx.x.x", // From version (e.g. v1.3.1)
    ...
    function _generateActions() internal override {
        ...
        _generateAction("LockLicenseHook-remove"); // remove from Module Registry
        _generateAction("LockLicenseHook-register"); // re-register in Module Registry
        _generateAction("TotalLicenseTokenLimitHook-remove"); // remove from Module Registry
        _generateAction("TotalLicenseTokenLimitHook-register"); // re-register in Module Registry
    }
